...
elseif isnumeric(b)
    ...
    % The right argument is numeric. We assume therefore that if we divide
    % components of the left argument by the right argument, that Matlab
    % will do the rest. Obviously if the right argument is zero, there will
    % be a divide by zero error. There may also be errors caused by type
    % mismatch (for example the components of a are double, but b is int8),
    % but Matlab will catch these.
...
% $Id: rdivide.m 1090 2020-06-15 17:17:19Z sangwine $
